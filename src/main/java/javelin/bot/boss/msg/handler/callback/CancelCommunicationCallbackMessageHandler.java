...
public class CancelCommunicationCallbackMessageHandler implements EntityCallbackMessageHandler {
    ...
    public HandleResult handle(CallbackCommand command) {
        var communication = communicationService.cancel((Long) command.getId());
        var txt = templateContext.processTemplate(
            TemplateNames.COMMUNICATION_CANCEL,
